...
linear_model_trend <- lm(inflow_data[,iHP+1]~t)
summary(linear_model_trend)

beta0 <- as.numeric(linear_model_trend$coefficients[1])
beta1 <- as.numeric(linear_model_trend$coefficients[2])

#detrended inflow

linear_trend <- beta0 + beta1  * t
detrended_inflow <- inflow_data[,iHP+1] - linear_trend
ts_detrend <- ts(detrended_inflow, start = c(1931,1), frequency = 12)
...
seasonal_lm <- lm(detrended_inflow ~ dummies)
summary(seasonal_lm)
